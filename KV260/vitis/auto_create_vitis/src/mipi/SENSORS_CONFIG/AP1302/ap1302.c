/*
   ...
   1.1   Sakinder 07/15/22 Added ap1302 Camera functions.
   -----------------------------------------------------------------------
*/
#include "ap1302.h"
#include <xil_types.h>
...
#define IIC_ap1302_ADDR  	        0x3C
#define ap1302_ANA_GAIN_GLOBAL      0x0157
#define ap1302_SENSOR_ID			0x0219
...
struct reginfo cfg_ap1302_1280_702p_60fps[] =
{
    {0x30EB, 0x05},
    {0x30EB, 0x0C},
    {0x300A, 0xFF},
...
struct reginfo cfg_ap1302_1920_1080p_60fps[] =
{
		{0x30eb, 0x05},
		{0x30eb, 0x0c},
		{0x300a, 0xff},
...
struct reginfo cfg2_ap1302_1920_1080p_60fps[] =
{
	    {0x0100, 0x00},
	    {0x30eb, 0x05},
	    {0x30eb, 0x0c},
...
struct reginfo cfg1_ap1302_1920_1080p_60fps[] =
{
        {REG_MODE_SEL,		       0x00},
...
int ap1302_read(XIicPs *IicInstance,u16 addr,u8 *read_buf)
{
	*read_buf=i2c_reg16_read(IicInstance,IIC_ap1302_ADDR,addr);
	return XST_SUCCESS;
}

int ap1302_write(XIicPs *IicInstance,u16 addr,u8 data)
{
	return i2c_reg16_write(IicInstance,IIC_ap1302_ADDR,addr,data);
}
void ap1302_sensor_write_array(XIicPs *IicInstance, struct reginfo *regarray)
{
	int i = 0;
	while (regarray[i].reg != SEQUENCE_END) {
		ap1302_write(IicInstance,regarray[i].reg,regarray[i].val);
		i++;
	}
}
int write_ap1302_camera_reg(XIicPs *IicInstance,u16 addr,u8 data)
{
    ap1302_write(IicInstance,addr,data);
    printf("ap1302 Reg Address = %x, Wrote Value = %x \n",addr,data);
}
int read_ap1302_camera_reg(XIicPs *IicInstance,u16 addr)
{
    int Status;
	u8 sensor_id[2];
	Status = ap1302_read(IicInstance,addr,&sensor_id[0]);
    printf("ap1302 Reg Address = %x, Read Value = %x \n",addr,sensor_id[0]);
}
int write_read_ap1302_camera_reg(XIicPs *IicInstance,u16 addr,u8 data)
{
    write_ap1302_camera_reg(IicInstance,addr,data);
    read_ap1302_camera_reg(IicInstance,addr);
}
int ap1302_camera_sensor_init(XIicPs *IicInstance)
{
	...
	if (sensor_id[0] == 0x2 || sensor_id[1] == 0x65)
	{
        ...
        //#else
        //ap1302_sensor_write_array(IicInstance,cfg_ap1302_1280_702p_60fps);
        //#endif
        //ap1302_write(IicInstance, ap1302_ANA_GAIN_GLOBAL, 50);
    	//ap1302_read(IicInstance, 0x0158, &sensor_id[0]);
    	//ap1302_read(IicInstance, 0x0159, &sensor_id[1]);
    	//printf("Read ap1302 id ap1302_ANA_GAIN_GLOBAL 0x0158: %x  id 0x0159: %x\n", sensor_id[0], sensor_id[1]);
        //ap1302_write(IicInstance, 0x2000, 0x280);
    	//ap1302_read(IicInstance, 0x2000, &sensor_id[0]);
    	//printf("Read ap1302 id 0x2000                   = %x\n",sensor_id[0]);
//
//    	ap1302_read(IicInstance, 0x0191, &sensor_id[0]);
//    	printf("Read ap1302 id 0x0191 LSC_COLOR_MODE_A              = %x\n",sensor_id[0]);
//
//    	ap1302_read(IicInstance, 0x0192, &sensor_id[0]);
//    	printf("Read ap1302 id 0x0192 LSC_SELECT_TABLE_A            = %x\n",sensor_id[0]);
//
//        //ap1302_write(IicInstance, 0x0193, 1);
//    	ap1302_read(IicInstance, 0x0193, &sensor_id[0]);
//    	printf("Read ap1302 id 0x0193 LSC_TUNING_ENABLE_A           = %x\n",sensor_id[0]);
//
//        ap1302_write(IicInstance, 0x0194, 0);
//    	ap1302_read(IicInstance, 0x0194, &sensor_id[0]);
//    	printf("Read ap1302 id 0x0194 LSC_WHITE_BALANCE_RG_A[15:8]  = %x\n",sensor_id[0]);
//
//        ap1302_write(IicInstance, 0x0195, 50);
//    	ap1302_read(IicInstance, 0x0195, &sensor_id[0]);
//    	printf("Read ap1302 id 0x0195 LSC_WHITE_BALANCE_RG_A[7:0]   = %x\n",sensor_id[0]);
//
//    	ap1302_read(IicInstance, 0x0198, &sensor_id[0]);
//    	printf("Read ap1302 id 0x0198 LSC_TUNING_COEF_R_A   = %x\n",sensor_id[0]);
//
//        ap1302_write(IicInstance, 0x0199, 50);
//    	ap1302_read(IicInstance, 0x0199, &sensor_id[0]);
//    	printf("Read ap1302 id 0x0199 LSC_TUNING_COEF_GR_A  = %x\n",sensor_id[0]);
//
//        ap1302_write(IicInstance, 0x019A, 50);
//    	ap1302_read(IicInstance, 0x019A, &sensor_id[0]);
//    	printf("Read ap1302 id 0x019A LSC_TUNING_COEF_GB_A  = %x\n",sensor_id[0]);
//
//
//    	ap1302_read(IicInstance, 0x019B, &sensor_id[0]);
//    	printf("Read ap1302 id 0x019B LSC_TUNING_COEF_B_A   = %x\n",sensor_id[0]);
//
//
//    	ap1302_read(IicInstance, 0x019C, &sensor_id[0]);
//    	printf("Read ap1302 id 0x019C LSC_TUNING_R_A[12:8]  = %x\n",sensor_id[0]);
//
//
//    	ap1302_read(IicInstance, 0x019D, &sensor_id[0]);
//    	printf("Read ap1302 id 0x019D LSC_TUNING_R_A[7:0]   = %x\n",sensor_id[0]);
//
//
//        ap1302_write(IicInstance, 0x019E, 0);
//    	ap1302_read(IicInstance, 0x019E, &sensor_id[0]);
//    	printf("Read ap1302 id 0x019E LSC_TUNING_GR_A[12:8] = %x\n",sensor_id[0]);
//
//
//        ap1302_write(IicInstance, 0x019F, 50);
//    	ap1302_read(IicInstance, 0x019F, &sensor_id[0]);
//    	printf("Read ap1302 id 0x019F LSC_TUNING_GR_A[7:0]  = %x\n",sensor_id[0]);
//
//        ap1302_write(IicInstance, 0x01A0, 0);
//    	ap1302_read(IicInstance, 0x01A0, &sensor_id[0]);
//    	printf("Read ap1302 id 0x01A0 LSC_TUNING_GB_A[12:8] = %x\n",sensor_id[0]);
//
//
//        ap1302_write(IicInstance, 0x01A1, 50);
//    	ap1302_read(IicInstance, 0x01A1, &sensor_id[0]);
//    	printf("Read ap1302 id 0x01A1 LSC_TUNING_GB_A[7:0]  = %x\n",sensor_id[0]);
//
//        ap1302_write(IicInstance, 0x01A2, 0);
//    	ap1302_read(IicInstance, 0x01A2, &sensor_id[0]);
//    	printf("Read ap1302 id 0x01A2 LSC_TUNING_B_A[12:8]  = %x\n",sensor_id[0]);
//
//        ap1302_write(IicInstance, 0x01A3, 50);
//    	ap1302_read(IicInstance, 0x01A3, &sensor_id[0]);
//    	printf("Read ap1302 id 0x01A3 LSC_TUNING_B_A[7:0]   = %x\n",sensor_id[0]);
	return XST_SUCCESS;
	}
}
...
{
	u8 sensor_id[1];
    ap1302_read(IicInstance, addr, &sensor_id[0]);
    printf("Read ap1302 Read Reg Address  =  %x   Value = %x\n",addr,sensor_id[0]);
	return 0;
}
int ap1302_write_register(XIicPs *IicInstance,u16 addr,u8 data)
{
	ap1302_write(IicInstance,REG_MODE_SEL,0x00);
	ap1302_write(IicInstance,addr,data);
	ap1302_write(IicInstance,REG_MODE_SEL,0x01);
    printf("Read ap1302 Write Reg Address  =  %x   Value = %x\n",addr,data);
	return 0;
}
int ap1302_write_read_register(XIicPs *IicInstance,u16 addr,u8 data)
{
	ap1302_write_register(IicInstance,addr,data);
	ap1302_read_register(IicInstance,addr);
	return 0;
}
